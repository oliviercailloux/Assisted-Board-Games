...
public class ChessMove {
	...
	/**public ChessMoveTXT ( String from, String To, Piece promotion) {
		move= new Move( Square.fromValue(from), Square.fromValue(To), promotion);
	...
	public static Move getMove(Board board1, Board board2) {
		return null;
	}**/

	public static JsonObject encode (Move move ) {
		JsonObject object = Json.createObjectBuilder()
				.add("Square From", move.getFrom().value() )
				.add("Square To", move.getTo().value())
				.add("Piece promotion", move.getPromotion().name())
				.build();
